...
struct LoadGraphSpec {
  ...
  // added by wenzhi
  void set_load_whole_graph(bool load_whole_graph){
    this->load_whole_graph_ = load_whole_graph;
    return;
  }
};
...
class BasicFragmentLoader<
 ...
 public:
  ...
  // added by wenzhi
  void SetLoadWholeGraph(bool load_whole_graph) {
    this->load_whole_graph_ = load_whole_graph;
    return;
  }
  ...
  void AddEdge(const oid_t& src, const oid_t& dst, const edata_t& data) {
    ...
    // if (kEachProcessLoadTotalGraph) {
    //   // add by wangyj: load total graph
    if (this->load_whole_graph_) {
      // modified by wenzhi, read argumet from class member
      for (fid_t i = 0; i < comm_spec_.fnum(); i++) {
        edges_to_frag_[i].Emplace(src, dst, ref_data);
      }
    } else {
