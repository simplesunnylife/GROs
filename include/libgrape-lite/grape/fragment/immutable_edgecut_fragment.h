 ...
 public:
  ...
  void Init(fid_t fid, std::vector<internal_vertex_t>& vertices,
    ...
    auto is_iv_gid = [this](VID_T id) { return (id >> fid_offset_) == fid_; };
    {
      ...
      auto first_iter_load_whole_graph =
          [&is_iv_gid, invalid_vid](Edge<VID_T, EDATA_T>& e,
                                    std::vector<VID_T>& outer_vertices) {
            if (is_iv_gid(e.src_)) {
              if (!is_iv_gid(e.dst_)) {
                outer_vertices.push_back(e.dst_);
      ...
      } else if (load_strategy == LoadStrategy::kLoadWholeGraph) {
        for (auto& e : edges) {
          first_iter_load_whole_graph(e, outer_vertices);
        }
      } else {
      ...
      if (load_strategy == LoadStrategy::kOnlyIn) {
        ...
        for (auto& e : edges) {
          second_iter_out(e, idegree, odegree);
                 ...
                 load_strategy == LoadStrategy::kLoadWholeGraph) {
        for (auto& e : edges) {
